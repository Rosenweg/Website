...
**Update (Heizstab-Steuerung):** Manuelle Modbus-Ansteuerung des SmartFox funktioniert NICHT (externe Modbus-Steuerung am Gerät nicht freigegeben, keine UI-Einstellung dafür; FW 00.01.09.09 ≠ Doku 00.01.03.10). Daher: Operator-Heizstab-Buttons sind **display-only** (onclick raus, `pointer-events:none`), Bridge-Script `script.js.Heizung.heizstab` **disabled**. PV-Automatik am SmartFox läuft normal. Reaktivieren, falls Steuerweg (z.B. via UI-API-Call) gefunden wird.

**ENTFERNT 2026-06-30:** Heizstab-Bridge komplett gelöscht (ioBroker-Script `script.js.Heizung.heizstab` + Python-Reste auf CT901) — SmartFox-Modbus-Ansteuerung ist nicht möglich (externe Steuerung am Gerät nicht freigegeben). Operator-Heizstab = nur Anzeige. heizstab-bridge.js nur noch im git-Verlauf, falls je ein Steuerweg gefunden wird.
