/*
 * Service Worker — Rosenweg "Meine Daten" (Selbstmutation) PWA
 * Scope: /daten/
 *
 * Strategie:
    ...
    event.respondWith(
      fetch(req)
        .then((resp) => {
          ...
          caches.open(CACHE).then((c) => c.put('/daten/index.html', copy)).catch(() => {});
          return resp;
        })
        .catch(() =>
          caches.match('/daten/index.html').then(
            (cached) => cached || caches.match('/daten/')
          )
        )
    );
    return;
