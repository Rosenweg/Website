/*
 * Service Worker — Rosenweg Arbeits-/Zeiterfassung PWA
 * Scope: /arbeit/
 *
 * Strategie:
 ...
 *  - /api/-Requests: NIE cachen — immer Netzwerk (Session/Live-Daten).
 *  - Sonstige GETs (Assets, CDN): stale-while-revalidate.
 */

const CACHE = 'rosenweg-arbeit-v11';
    ...
    event.respondWith(
      fetch(req)
        .then((resp) => {
          ...
          caches.open(CACHE).then((c) => c.put('/arbeit/index.html', copy)).catch(() => {});
          return resp;
        })
        .catch(() =>
          caches.match('/arbeit/index.html').then(
            (cached) => cached || caches.match('/arbeit/')
          )
        )
    );
    return;
